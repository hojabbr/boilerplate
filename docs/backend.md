...
- **Core** (`app/Core/`) — Cross-cutting: Middleware, Models (Language, Setting, FeatureFlag), Observers, Policies, Providers, Services (PagePropsService, SupportedLocalesService), Contracts, Exceptions, Traits. **Core/Inertia/** contains `TestingViewFinder` for resolving Inertia page component names in PHP tests.
- **Domains** (`app/Domains/<Name>/`) — Vertical slices: Auth, Blog, Contact, Dashboard, Faq, Landing, Page, Profile, Search, Testimonial. Each may have Http/Controllers, Http/Requests, Models, Observers, Policies, Queries, Services, Actions, DTOs, Search, Jobs/. Example: Blog has `Jobs/` (e.g. `GenerateBlogPostsJob`) and `Services/` (e.g. `BlogPostGenerationService`) for queued AI-powered post generation.
