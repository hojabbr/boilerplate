...
Two interactive commands scaffold boilerplate so backend, Filament, and frontend stay in sync. They use [Laravel Prompts](https://laravel.com/docs/12.x/prompts) for input and output.

- **`php artisan boilerplate:locale`** — Add a new locale end-to-end. You are prompted for: locale code (e.g. `pt`, `hi`), metadata source (pick from commented lines in `config/laravellocalization.php` or enter name/script/native/regional manually), whether the language is RTL, and whether to add placeholder entries to SettingSeeder, PageSeeder, and LandingSectionSeeder. The command updates `config/laravellocalization.php`, `resources/js/i18n/index.ts`, optionally `resources/js/app.tsx` (RTL), creates `lang/{code}.json` from `lang/en.json`, and optionally adds the new locale to the three seeders. **Options:** `--dry-run` shows planned changes without applying them; `--rollback=<code>` removes the locale from config, i18n, lang file, seeders, and RTL list. **After running:** `php artisan wayfinder:generate`, and if you chose seeder updates then `php artisan db:seed`. Translate `lang/{code}.json` and seeder content as needed.

- **`php artisan boilerplate:domain`** — Scaffold a new domain (vertical slice). You are prompted for: domain name (PascalCase, **singular**, e.g. `Product`, `Page`), route path (kebab-case), whether to create an Eloquent model (and if so, table name, “create migration first” vs “command generates migration”, Spatie Translatable, Scout/Meilisearch), whether to create a Filament resource (List/Create/Edit only), whether to gate with a Pennant feature flag, and whether to create the frontend feature module. The command creates `app/Domains/{Name}/Http/Controllers/`, registers the route in the locale group, and optionally the model (with migration or from an existing migration), Scout config entry, Filament resource and permission, feature class (and AppServiceProvider + `config/features.php`), and `resources/js/features/{path}/` with a placeholder Index page. **Options:** `--dry-run` lists planned actions without creating or editing files; `--rollback=<Name>` removes the scaffolded domain (controller, route, model, migration, Filament resource, feature class, config edits, frontend module). **After running:** `php artisan wayfinder:generate`, run migrations if a new migration was added, and if the model is searchable run `php artisan scout:import "App\Domains\...\Model"`. If you added a feature flag, add it to `config/features.php` toggleable list if needed and run `php artisan db:seed --class=FeatureFlagSeeder`.

Relation managers and custom Filament pages are not generated by the domain command; add them manually as described in this doc.

**Keeping scaffolding in sync:** When you change project structure or conventions that affect what the boilerplate commands generate, update the commands and [tests/Feature/BoilerplateCommandsTest.php](tests/Feature/BoilerplateCommandsTest.php), and run those tests. The test file defines the contract for non-interactive behavior, rollback, and dry-run; when changing the architecture or the commands, run and update these tests so they still pass and reflect the intended behavior.

### New domain

Use a **singular** name for the domain (e.g. `Page`, `Product`, not `Pages`, `Products`). The boilerplate command enforces this.
